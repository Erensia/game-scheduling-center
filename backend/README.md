...
1. `File → Import... → Gradle → Existing Gradle Project` 선택 → `backend` 폴더 지정 → Import
   (STS 내장 Buildship이 자체 Gradle로 자동 동기화합니다. 인터넷 연결 필요 — Maven Central에서 의존성을 받아옵니다.)
...
gradle wrapper --gradle-version 8.14
```
(Windows라면 SDKMAN 대신 zip 압축 해제 후 시스템 환경 변수 PATH에 `...\gradle-8.14\bin` 추가하는 방식이 가장 간단해요.)
